...
export const useRelacionesStore = defineStore("relacionesStore", () => {
    ...
    try {
      ...
      if (relacion.id) {
        ...
        response = await fetch(`https://localhost:7163/api/UsuarioTutores/${relacion.id}`, {
          ...
          body: JSON.stringify({ idUsuario: relacion.idUsuario, idTutor: relacion.idTutor }),
        });
      } else {
        // POST
  ...
  const eliminarRelacion = async (id: number) => {
    try {
      const res = await fetch(`https://localhost:7163/api/UsuarioTutores/${id}`, { method: "DELETE" });
